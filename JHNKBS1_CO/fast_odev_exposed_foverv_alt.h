...
for (i=m+n+7;i<=m+n+6+m-1;i++){
    *(dfdy + m+n+1 * DIM +i) = 0;
}
*(dfdy + m+n+1 * DIM + m+n+6+m) = m*lambdaF*size_C*(1-coinf_V);
...
*(dfdy + m+n+2 * DIM + m+n+6+m) = indexR*m*lambdaF*(1-coinf_V);
...
*(dfdy + m+n+3 * DIM + m+n+6+m) = m*lambdaF*coinf_V;
...
*(dfdy + m+n+4 * DIM + m+n+6+m) = m*lambdaF*coinf_V;
...
*(dfdy + m+n+5 * DIM + m+n+6+m) =  m*lambdaF*(1-coinf_V);
...
*(dfdy + m+n+6 * DIM + m+n+6+m) = indexV*m*lambdaF*coinf_V;

*(dfdy + m+n+6+1 * DIM +0) = 0; // \partial (dEf1/dt) \partial S
*(dfdy + m+n+6+1 * DIM +1) = 0;  // \partial (dEf1/dt) \partial Ef1
for(i=2; i <= m; i++){              // \partial (dEf1/dt) \partial Ef, i > 2
	*(dfdy + m+n+6+1 * DIM +i) = 0;
}
for(i=m+1; i <= m+n1; i++){              // \partial (dEf1/dt) \partial Evf
	*(dfdy + 1 * DIM +i) = (nuF*y[m+n+1] + nuR*R)*VFSus;
	//*(dfdy + m+n+6+1 * DIM +i) = 0;
...
*(dfdy + m+n+6+1 * DIM + m + n + 1) = nuF*y[0]; // \partial (dE1/dt) \partial C
for(i=m+1; i <= m+n1; i++){
        //*(dfdy + 1 * DIM + m + n + 1) += nuF*y[i]*VFSus;
        *(dfdy + m+n+6+1 * DIM + m + n + 1) += 0;
}
*(dfdy + m+n+6+1 * DIM + m + n + 2) = 0; // \partial (dE1/dt) \partial R
*(dfdy + m+n+6+1 * DIM + m + n + 3) = 0; // \partial (dE1/dt) \partial P
*(dfdy + m+n+6+1 * DIM + m + n + 4) = 0;
*(dfdy + m+n+6+1 * DIM + m + n + 5) = 0;
*(dfdy + m+n+6+1 * DIM + m + n + 6) = 0;
*(dfdy + m+n+6+1 * DIM + m + n + 6 +1) = -m*lambdaF;
for(i=2; i <= m; i++){               //\partial (dS/dt) \partial E_i
	*(dfdy + m+n+6+1 * DIM + m + n + 6+ i) = 0;
}

for(a=m+n+6+2; a <= m+n+6+m; a++){
	for(b=0; b <= m+n+6+m; b++){
		if(b == (a-1)){*(dfdy + a * DIM +b) =  m*lambdaF;} // \partial (dEfi/dt) \partial Ef(i-1)
		else if (b == a){*(dfdy + a * DIM +b) = -m*lambdaF;}  // \partial (dEfi/dt) \partial Efi
 		else {*(dfdy + a * DIM +b) = 0;}
