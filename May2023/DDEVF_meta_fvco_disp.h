...
//m
int m_l_pop_fit = 1; //set to 1 for pop level m
int m_l_meta_fit = 0; //set to 1 for meta level m 
			...
			if(larvae_dispersal_on == 1){
				...
				for(subout=0; subout<num_sub; subout++){
					...
					for (i=1;i<=n1;i++)	{ //for each exposed class
						...
						//SCALE OF MIGRATION
						VFout[subout][i] += exp(-a_l[j][subout]*10) * E_VF[subout][i];
						...
						for(subin=0; subin<num_sub; subin++){
							...
							if(subin!=subout){
								...
								VFin[subin][i] += VFout[subout][i]*m_l[j][subout]*exp(-a_l[j][subout]*Params->DISTANCE[j][subout][subin]);
							}
						}
					}
					for (ii=1;ii<=n2;ii++)	{ //ii not i
						...
						Vout[subout][ii] += exp(-a_l[j][subout]*10) * E_V[subout][ii];
						...
						for(subin=0; subin<num_sub; subin++){
							...
							if(subin!=subout){
								...
								Vin[subin][ii] += Vout[subin][ii]*m_l[j][subout] * exp(-a_l[j][subout]*Params->DISTANCE[j][subout][subin]);
							}
						}
					}
				}
...
//update state variables
	for(sub=0; sub<num_sub; sub++){
		...
		V[sub]=y_ode[m+n+3+sub_index[sub]]; //OBS
	...
	for(sub=0; sub<num_sub; sub++){	
		...
		IV[sub]=0; //EARLY VIRUS INFECTED
		IVF[sub]=0; //LATE VIRUS INFECTED
