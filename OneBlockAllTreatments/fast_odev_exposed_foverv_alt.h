...
for(tmt=0, tmt<4, tmt++;){
	...
	dydt[m+1+tmt_index[tmt]] = y[0+tmt_index[tmt]]*nuV*y[m+n+3+tmt_index[tmt]]*pow((y[0+tmt_index[tmt]]/S[tmt]),squareCVV)-n*lambdaV*y[m+1+tmt_index[tmt]]-y[m+1+tmt_index[tmt]]*(nuF*y[m+n+1+tmt_index[tmt]] + nuR*R)*VFSus;
	for (i=2;i<=n1;i++){
